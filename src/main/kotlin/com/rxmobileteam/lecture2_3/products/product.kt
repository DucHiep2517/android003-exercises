...
fun List<Order>.getDeliveredProductsList(): List<Product> {
    return this.getDeliveredOrders().getProductsList()
}
...
fun List<Order>.partitionDeliveredAndNotDelivered(): Pair<List<Order>, List<Order>> {
    val (deliveredOrders, notDeliveredOrders) = this.partition { it.isDelivered }
    return Pair(deliveredOrders, notDeliveredOrders)
}
